...
//
// GRDB+SQLCipher fork for fekirtech POS
// Based on upstream GRDB.swift with SQLCipher enabled per inline comments.
...
// GRDB+SQLCipher: SQLCipher dependency and defines
...
let package = Package(
    ...
    targets: [
        // GRDB+SQLCipher: GRDBSQLCipher target (bridges SQLCipher headers)
        .target(
            name: "GRDBSQLCipher",
        ...
        .target(
            name: "GRDB",
            dependencies: [
                // GRDB+SQLCipher: Use SQLCipher instead of system SQLite
